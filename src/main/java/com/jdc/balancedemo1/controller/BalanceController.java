...
public class BalanceController {
	...
	@GetMapping
	String report(ModelMap model, @RequestParam(required = false) Type type,
		...
		var pagination = Pagination.builder("/user/balance")
				.params(Map.of("type", null == type ? "" : type.name(), "dateFrom",
						null == dateFrom ? "" : dateFrom.format(dateFormatter), "dateTo",
						null == dateTo ? "" : dateTo.format(dateFormatter)))
				.page(result).build();
	...
	@GetMapping("{type}")
	String searchBalanceItems(ModelMap model, @PathVariable Type type,
			@RequestParam(required = false) @DateTimeFormat(pattern = "yyyy-MM-dd") LocalDate dateFrom,
			@RequestParam(required = false) @DateTimeFormat(pattern = "yyyy-MM-dd") LocalDate dateTo,
			@RequestParam(required = false) String keyword, @RequestParam Optional<Integer> page,
		...
		var pagination = Pagination.builder("/user/balance/%s".formatted(type)).page(result).params(params).build();
		model.put("pagination", pagination);
